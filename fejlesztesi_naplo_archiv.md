# Kiszedési Jegyzék Shopify - Fejlesztési Napló Archívum (v1.0 - v4.5.9)

Ez a dokumentum a v4.6.0 előtti korábbi fejlesztések részletes archívuma.

---

### 2026. szeptember 21. (2. frissítés) - Terítés Törlés Képernyőn Maradási Hiba Végleges Megszüntetése (`v4.5.9`)
- **Gyökérok (Firestore Eventual Consistency Race Condition)**:
  - A törlés lefutásakor a kód korábban meghívta a `this.invalidateCache()` eljárást, ami azonnal `null`-ra állította a memóriabeli gyorsítótárat (`runsCache = null`).
  - Emiatt a nézet újrarajzolásakor a rendszer azonnal új `getDocs()` lekérdezést küldött a Firestore felé. Mivel a Firestore szerveroldali indexei 500-1500 ms késleltetéssel frissülnek a törlés után, a túl korai szerverlekérdezés még visszaküldte a frissen törölt dokumentumot, így a képernyőn visszarajzolódott a kártya. Amikor a felhasználó átváltott egy másik fülre és vissza, addigra a Firestore szerver is lefrissült, és eltűnt a kör.
- **Megoldás (`js/services/history.js`, `js/app.js`)**:
  - **Szűrt gyorsítótár megőrzése**: Törléskor NEM nullázzuk a gyorsítótárat (`runsCache`), hanem közvetlenül kiveszzük belőle a törölt kört és frissítjük a TTL időbélyeget (`lastRunsFetchTime = Date.now()`). Így az azonnali újrarajzolás garantáltan a törölt elem nélküli tiszta tömbből dolgozik, megakadályozva az indexelési késleltetésből adódó visszarajzolódást.
  - **DOM elem és Cégcsoport Azonnali Eltávolítása**: Ha egy cégnek az adott napon csak 1 köre volt, a kód azonnal eltávolítja a teljes befoglaló cégcsoportot (`accounting-company-group`) a DOM-ból.
  - **Blokkoló Alert Modális Ablak Kivezetése**: Megszüntettük a törlés után felugró felesleges jóváhagyó ablakot, amely kitakarta a képernyőt és blokkolta az animációt.
- **Cache-Busting és verziókezelés**:
  - Verziószám megemelve `v=4.5.9`-re (`index.html`, `historyView.js`, `package.json`).
