...
- **PannonXP Számlázási oszlopok ürítése**: A PannonXP CSV exportőrben a számlázási mezőket (`szl_koltsegviselo`, `szl_adoszam`, `szl_maganszemely`) üresen hagyjuk. Korábban a hiányzó feladói adószám miatt a PannonXP szoftver `HIBÁS ADAT!` jelzéssel elutasította a teljes importot; az üres mezőkkel a PannonXP automatikusan a fiókodhoz rendelt alapértelmezett számlázási adatokat fogja használni hiba nélkül.
- **CSV elcsúszás gátló (Semicolon & Newline strip)**: Továbbfejlesztettük a CSV formázót (`escapeCsvValue`), hogy minden szövegből automatikusan távolítsa el a pontosvesszőket és sortöréseket. Ezzel megelőzzük, hogy a vásárlók által írt megjegyzések vagy címek eltolják az oszlopokat a PannonXP desktop szoftverben.
- **Cache-Busting és verziókezelés**: Megemeltük az érintett fájlok importálási cache verzióit `v190`-re a böngészők frissítésének kikényszerítéséhez.
