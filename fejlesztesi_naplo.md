...
- **Sortörések és pontosvesszők automatikus szűrése a CSV-ben**: Kiderült, hogy a beállításokban szereplő „Tartalom” (szl_tartalom) vagy a vevőcímek/jegyzetek sortörést (új sort) tartalmaztak, ami elvágta a CSV sorát és eltolta az adatokat (pl. az utánvét összege vagy adószáma rossz oszlopba került a PannonXP-ben, ami hibás adat jelzést eredményezett). Bevezettünk egy automatikus szűrőt (`escapeCsvValue`), ami a pontosvesszőket és sortöréseket szóközre cseréli, megelőzve az elcsúszást.
- **Cache-Busting és verziókezelés**: Megemeltük az érintett fájlok importálási cache verzióit `v192`-re a böngészők frissítésének kikényszerítéséhez.
